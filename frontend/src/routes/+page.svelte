<script lang="ts">
	...
	// Svelte 5 reactivity for debouncing
	$effect(() => {
		// This code runs automatically whenever 'searchTerm' changes
		const currentSearchTerm = searchTerm;
		clearTimeout(debounceTimeout);

		// Only set a timer if there's something to search for
		...
		// This is a cleanup function that runs when the component is destroyed
		return () => {
			clearTimeout(debounceTimeout);
		};
	});
...
>
	...
	<div class="w-full max-w-4xl">
		...
		{:else if error}
			<p class="rounded-xl bg-red-100 p-4 text-center text-red-500">{error}</p>
